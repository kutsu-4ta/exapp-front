...
export async function fetchSprintAiContext(): Promise<string> {
  ...
  lines.push('## 活用テンプレート')
  lines.push('')
  lines.push('### スプリントプランニング依頼')
  lines.push('```')
  lines.push('上記のデータを元に次のスプリント計画を提案してください。')
  lines.push('- バックログから優先すべきチケットを選んでください')
  lines.push('- スプリント期間は1〜2週間を想定')
  lines.push('- 適切なチケット数（目安5〜10件）を選定してください')
  lines.push('- スプリント目標（ゴール）を一文で提案してください')
  lines.push('- 各チケットを選んだ理由・優先順位の根拠を添えてください')
  lines.push('```')
  lines.push('')
  lines.push('### チケット作成テンプレート')
  lines.push('```')
  lines.push('subject: [科目名]')
  lines.push('title: [学習タスクのタイトル]')
  lines.push('acceptanceCriteria: [完了の定義・完了条件]')
  lines.push('dueDate: [YYYY-MM-DD]')
  lines.push('priority: high（高） / medium（中） / low（低）')
  lines.push('ticketType: knowledge（知識） / practice（演習） / understanding（理解） / memorization（暗記）')
  lines.push('source: wrong_answer（過去問ミス） / mock_exam（模試） / review（見直し） / manual（手動）')
  lines.push('estimateMinutes: [見積もり分数 例: 30]')
  lines.push('```')
  lines.push('')
  lines.push('### チケット一括作成依頼')
  lines.push('```')
  lines.push('以下のテーマで学習チケットを複数作成してください。')
  lines.push('テーマ: [ここに記入]')
  lines.push('件数: [例: 5件]')
  lines.push('上記のチケット作成テンプレート形式でそれぞれ出力してください。')
  lines.push('```')
