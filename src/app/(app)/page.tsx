...
export function FailureAnalysisSection({ failureData }: { failureData: { type: string, count: number }[] }) {
        ...
        <section style={sectionContainer}>
            <div style={sectionHeader}>
                ...
                <div style={toggleContainer}>
                    <button onClick={() => setViewMode('bar')} style={{...toggleIconBtn, color: viewMode === 'bar' ? '#37352f' : 'rgba(55, 53, 47, 0.3)', backgroundColor: viewMode === 'bar' ? 'rgba(55, 53, 47, 0.08)' : 'transparent'}}>📊</button>
                    <button onClick={() => setViewMode('pie')} style={{...toggleIconBtn, color: viewMode === 'pie' ? '#37352f' : 'rgba(55, 53, 47, 0.3)', backgroundColor: viewMode === 'pie' ? 'rgba(55, 53, 47, 0.08)' : 'transparent'}}>🍩</button>
                </div>
            </div>
                ...
                {viewMode === 'bar' ? (
                    failureData.map(({ type, count }) => (
                        <div key={type} style={analysisRow}>
                            <div style={analysisHeader}><span style={analysisText}>{type}</span><span style={countText}>{count} cases</span></div>
                            <div style={progressBarBg}>
                                <div style={{...progressBarFill, width: `${(count / (Math.max(...failureData.map(d => d.count)) || 1)) * 100}%`, backgroundColor: count > 10 ? '#eb5757' : '#37352f'}} />
                            </div>
                        </div>
                    ))
                ) : (
...
const pieFrame: React.CSSProperties = {
    position: 'relative',
    width: '180px',
    height: '180px'
}

const pieChart: React.CSSProperties = {
    width: '100%',
    height: '100%',
    borderRadius: '50%',
...
const ratioList: React.CSSProperties = {
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
    padding: '16px',
...
const ratioInfo: React.CSSProperties = {
    display: 'flex',
    alignItems: 'baseline',
    gap: '12px'
}

const ratioValue: React.CSSProperties = {
    fontSize: '18px',
    fontWeight: 700,
    color: '#2383e2', // 目立つ色で割合を表示
    width: '60px'
}

const totalInfo: React.CSSProperties = {
    marginTop: '12px',
    paddingTop: '12px',
