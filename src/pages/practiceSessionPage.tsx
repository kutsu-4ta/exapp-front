...
export default function PracticeSessionPage() {
  ...
  const handleAddSubQuestion = () => {
    setSubQuestions((prev) => [...prev, makeSubQuestion(currentIndex, prev.length + 1)])
  }
  ...
  const handleGoBack = () => {
    ...
    setSubQuestions(
      prev.answers.length > 1 ? prev.answers.map((_, i) => makeSubQuestion(prev.index, i + 1)) : []
    )
    setQuestionStartMs(Date.now())
    setPendingInitAnswers(
      prev.answers.map((a) => ({
        answer: a.answer,
