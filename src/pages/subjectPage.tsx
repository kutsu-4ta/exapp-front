...
import type {Flashcard, SubCategory, SubjectActivityDay, SubjectSettings,} from '../types/workspace'
...
export default function SubjectPage() {
  ...
  const handleGoalSave = async () => {
    setGoalSaving(true)
    try {
      const saved = await saveSubjectMonthlyGoal(
        subjectName,
        viewYear,
        viewMonth,
        monthlyGoal || null
  ...
  const handleTitleRename = async () => {
    ...
    try {
      ...
      setSubCategories(
        subCategories.map((sc) => (sc.subject === subjectName ? { ...sc, subject: newName } : sc))
      )
      setIsEditingTitle(false)
      ...
      <div style={contentWrap}>
        ...
        <div style={titleContainer}>
          {isEditingTitle ? (
            ...
            <h1 style={heading} onClick={() => setIsEditingTitle(true)} title="クリックして編集">
        ...
        <div style={block}>
          ...
          {!settingsLoaded ? (
            ...
            <div style={strategyRow}>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((s) => ({ ...s, finalTarget: e.target.value || null }))
                }
          ...
          <div style={{ marginTop: '8px' }}>
            <div style={monthNav}>
              <button style={monthNavBtn} onClick={prevMonth}>
                ‹
              </button>
              ...
              <button style={monthNavBtn} onClick={nextMonth}>
                ›
              </button>
            </div>
              ...
              <div style={block}>
                ...
                {examStats.rankStats.length > 0 && (
                  <>
                    ...
                    >
                      {RANKS.filter((r) => examStats.rankStats.some((rs) => rs.rank === r)).map(
                        (rank) => {
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {examStats.recentMistakes.slice(0, 8).map((m) => (
                        <div key={`${m.sessionId}-${m.questionId}`} style={mistakeRow}>
                          <span style={{ ...rankBadge, ...RANK_COLORS[m.rank], flexShrink: 0 }}>
                            {m.rank}
                          </span>
                          <span style={mistakeMeta}>
                            {m.examYear} {m.displayId}
                          </span>
        ...
        <details style={settingsCollapse}>
          ...
          <div style={{ marginTop: '32px' }}>
                  ...
                  <div key={sc.id}>
                    {editingId === sc.id ? (
                      <div style={editRow}>
                        ...
                        />
                        <button onClick={handleEditSave} disabled={editLoading} style={saveBtn}>
                          保存
                        </button>
                      </div>
                      ...
                      <div style={deleteConfirmRow}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                          ...
                          <button onClick={() => setDeleteSubCatTarget(null)} style={cancelBtn}>
                            中止
                          </button>
                        </div>
                      </div>
                      ...
                      <div style={itemRow}>
                        ...
                        <div style={itemActions}>
                          ...
                          <button
                            onClick={() => setDeleteSubCatTarget(sc)}
                            style={{ ...iconBtn, color: 'rgba(235, 87, 87, 0.6)' }}
                          >
                            削除
                          </button>
                        </div>
            ...
            <div style={dangerBlock}>
              ...
              {deleteConfirming ? (
                <div style={deleteConfirmRow}>
                  <button onClick={handleDelete} disabled={deleteLoading} style={destructiveBtn}>
                    削除を確定
                  </button>
                  <button onClick={() => setDeleteConfirming(false)} style={cancelBtn}>
                    戻る
                  </button>
                </div>
              ) : (
                <button onClick={() => setDeleteConfirming(true)} style={dangerBtn}>
                  科目を削除する
                </button>
              )}
            </div>
...
// TODAY'S 5
const todayCardRow: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '10px',
  padding: '10px 0',
...
const todayCardSub: React.CSSProperties = {
  fontSize: '11px',
...
const monthNavLabel: React.CSSProperties = {
  fontSize: '13px',
  fontWeight: 700,
  color: c.text,
  display: 'flex',
...
const currentMonthBadge: React.CSSProperties = {
  fontSize: '10px',
  color: c.blue,
...
const dangerSectionHeading: React.CSSProperties = {
  ...sectionHeading,
...
const mistakeMeta: React.CSSProperties = {
  fontSize: '11px',
  color: 'rgba(55,53,47,0.4)',
  flexShrink: 0,
}
const mistakeNote: React.CSSProperties = {
  fontSize: '11px',
  color: c.text,
  flex: 1,
  overflow: 'hidden',
...
const settingsCollapse: React.CSSProperties = {
  marginTop: '64px',
  borderTop: `1px solid rgba(55, 53, 47, 0.08)`,
  paddingTop: '24px',
}
const settingsSummary: React.CSSProperties = {
  fontSize: '12px',
  color: 'rgba(55, 53, 47, 0.3)',
  cursor: 'pointer',
  textAlign: 'center',
...
const addRow: React.CSSProperties = {
  ...
  gap: '8px',
  marginTop: '12px',
  borderTop: '1px solid rgba(55,53,47,0.05)',
  paddingTop: '12px',
}
...
const dangerBlock: React.CSSProperties = {
  ...
  borderRadius: '10px',
  padding: '20px',
  backgroundColor: 'rgba(235, 87, 87, 0.01)',
}
