...
export function SubjectActivityChart({ data, year, month }: Props) {
    ...
    const rows: ChartRow[] = Array.from({ length: daysInMonth }, (_, i) => {
        ...
        return { day, date, studyMinutes: entry?.studyMinutes ?? 0, problemCount: entry?.problemCount ?? 0 }
    })
...
function ActivityTooltip({ active, payload }: { active?: boolean; payload?: { payload: ChartRow }[] }) {
    if (!active || !payload?.length) return null
    const d = payload[0].payload
    if (d.studyMinutes === 0 && d.problemCount === 0) return null
    return (
        <div style={{
            background: '#fff', border: '1px solid rgba(55,53,47,0.1)',
