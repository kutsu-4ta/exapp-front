...
function computeSubjectCards(sessions: ExamSessionSummary[]): SubjectCardData[] {
  return SUBJECTS.map(subject => {
    ...
    const status: SubjectCardData['status'] = avgPure >= 60 ? '安定' : avgPure >= 50 ? '注意' : '要強化'
...
export default function AnalysisView() {
  ...
  const chartData = useMemo(() => {
    const filtered = filterSubject === 'すべて'
      ? sessions
          ...
          <div style={{ width: '100%', height: 240 }}>
            <ResponsiveContainer>
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <ReferenceLine y={60} stroke="#2383e2" strokeDasharray="5 5"
...
const subjStatusTag: React.CSSProperties = { fontSize: '9px', fontWeight: 800, padding: '2px 6px', borderRadius: '4px' }
