...
export function FailureAnalysisSection({ problems, subjects }: Props) {
    const rows = useMemo(() =>
        subjects.map(subject => {
            const sp = problems.filter(p => p.subject === subject && (p.proficiency === '△' || p.proficiency === '×'))
    ...
    return (
        <section className="mb-12">
                ...
                <div className="flex gap-3">
                    {FAILURE_TYPE_VALUES.map(ft => (
                        <div key={ft} className="flex items-center gap-1.5">
                            <div className="w-2 h-2 rounded-[2px] shrink-0" style={{ backgroundColor: FAILURE_COLORS[ft] }} />
            ...
            {rows.length === 0 ? (
                <p className="text-[13px] text-[var(--nt-text-hint)] text-center py-6">データがありません</p>
            ) : (
                    ...
                    {rows.map(({ subject, counts, total, problemCount }) => (
                        <div key={subject}>
                            ...
                            <div className="flex gap-3 mt-1.5">
                                {counts.filter(c => c.count > 0).map(({ type, count }) => (
                                    <span key={type} className="text-[11px] text-[var(--nt-text-sub)]">
                                        <span style={{ color: FAILURE_COLORS[type] }} className="font-semibold">{type}</span>
