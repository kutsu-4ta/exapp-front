...
export function AiTrafficIndicator() {
  ...
  const chipColor = isLimited ? '#eb5757' : rpdColor(rpdPct)
      ...
      {hovering && (
        <div style={tooltipBox}>
          ...
          <div style={{ marginBottom: '10px' }}>
            ...
            {rpdLimited && <p style={warningText}>本日の上限に達しました</p>}
            {!rpdLimited && today > 0 && <p style={hintText}>残り {rpdLimit - today} 回</p>}
            {today === 0 && <p style={hintText}>{rpdLimit} 回 / 日</p>}
          </div>
          ...
          <div style={{ marginTop: '8px' }}>
            ...
            {rpmLimited && rpmMs !== null && (
              <p style={recoveringText}>{Math.ceil(rpmMs / 1000)}s後に回復</p>
            )}
            {!rpmLimited && lastMin > 0 && (
              <p style={hintText}>残り {rpmLimit - lastMin} 回 / 分</p>
            )}
            {lastMin === 0 && <p style={hintText}>{rpmLimit} 回 / 分</p>}
          </div>
