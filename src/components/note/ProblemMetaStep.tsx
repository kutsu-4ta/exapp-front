...
export function ProblemMetaStep({ initial, subCategories, onCancel, onNext }: Props) {
  ...
  return (
    <div style={editableRow}>
      ...
      >
        <button onClick={onCancel} style={notionDeleteBtn}>
          キャンセル
        </button>

        <button onClick={handleNext} disabled={loading} style={notionSaveBtn}>
          {loading ? '作成中...' : '次へ'}
        </button>
      </div>
    </div>
