...
export function NoteToTicketsModal({ problems, allSubCategories, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    problems.forEach((p) => {
      ...
      if (autoId !== null) { map[p.id] = [autoId]; resolved.add(p.id) }
      else { map[p.id] = [] }
    })
    setSubCatMap(map)
  ...
  const grouped = useMemo(() => {
    ...
    return subjects.map((subj) => ({ subject: subj, items: problems.filter((p) => p.subject === subj) }))
  }, [problems])
  ...
  const toggleAllInGroup = (items: Problem[]) => {
    ...
    setSelectedIds((prev) => { const next = new Set(prev); items.forEach((p) => allSelected ? next.delete(p.id) : next.add(p.id)); return next })
  }

  const toggleSubCat = (problemId: number, subCatId: number) => {
    setSubCatMap((prev) => {
      const cur = prev[problemId] ?? []
      const next = cur.includes(subCatId) ? cur.filter((id) => id !== subCatId) : [...cur, subCatId]
      return { ...prev, [problemId]: next }
    })
    setAutoResolvedIds((prev) => { const next = new Set(prev); next.delete(problemId); return next })
  }
              ...
              return (
                <div key={subject}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <button style={toggleBtn} onClick={() => toggleAllInGroup(items)}>
                      {allSelected ? '全解除' : '全選択'}
                    </button>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {items.map((p) => {
                      ...
                      return (
                        <div key={p.id} style={{ borderRadius: 8, border: `1px solid ${isUnassigned ? c.redBorder : c.border}`, backgroundColor: isSelected ? c.bg : 'rgba(55,53,47,0.02)', opacity: isSelected ? 1 : 0.5, overflow: 'hidden' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', cursor: 'pointer' }} onClick={() => toggleProblem(p.id)}>
                            <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, marginTop: 2, border: isSelected ? 'none' : `1.5px solid ${c.border}`, background: isSelected ? c.blue : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
                                <span style={{ fontSize: '13px', fontWeight: 700, color: c.text }}>{p.questionRef ?? '（問題参照なし）'}</span>
                                <span style={{ fontSize: font.xs, fontWeight: 700, color: PROFICIENCY_COLOR[p.proficiency] ?? c.textSub, border: `1px solid ${PROFICIENCY_COLOR[p.proficiency] ?? c.border}`, borderRadius: 4, padding: '1px 5px' }}>{p.proficiency}</span>
                                {p.failureTypes.map((ft) => (
                                  <span key={ft} style={{ fontSize: font.xs, color: c.textSub, backgroundColor: 'rgba(55,53,47,0.06)', borderRadius: 4, padding: '1px 6px' }}>{ft}</span>
                                ))}
                                <span style={{ fontSize: font.xs, color: c.blue, backgroundColor: 'rgba(35,131,226,0.07)', borderRadius: 4, padding: '1px 6px' }}>{ticketType}</span>
                              </div>
                              {p.note && <p style={{ margin: 0, fontSize: font.sm, color: c.textSub, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{p.note}</p>}
                            </div>
                          </div>
                          {isSelected && (
                              ...
                              {subjectSubCats.length === 0 ? (
                                ...
                                <>
                                  {subjectSubCats.map((sc) => {
                                    ...
                                    return (
                                      <button key={sc.id} style={{ padding: '4px 12px', borderRadius: 20, border: active ? 'none' : `1px solid ${c.border}`, background: active ? c.blue : 'transparent', color: active ? '#fff' : c.textSub, fontSize: font.sm, fontWeight: active ? 700 : 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }} onClick={() => toggleSubCat(p.id, sc.id)}>
                                        {sc.name}
                                        {isAuto && <span style={{ fontSize: font.xs, opacity: 0.8 }}>自動</span>}
                                      </button>
                                    )
                                  })}
